<?php
    ...
    if(empty($_SESSION['_iduser'])){
        ...
        function ubahformatTgl_RP($tanggal) {
            $pisah = explode('/',$tanggal);
            return $pisah[2].'-'.$pisah[1].'-'.$pisah[0];
        }

        include_once "includes/report-default-range.php";
        $_default_range = app_report_default_range($koneksi, 'tblretur_pembelian_header', 'tanggal', "kd_cabang='$kd_cabang'");
        ...
        $sql_query = "SELECT h.noretur, h.nopembelian, DATE_FORMAT(h.tanggal,'%d/%m/%Y') AS tanggal_trx,
                             h.total_qty_retur, h.total_akhir, h.status_retur, h.note,
                             COALESCE(s.namasupplier,'') AS namasupplier
        ...
        $cari_tot = mysqli_query($koneksi,"SELECT COUNT(*) AS tot FROM tblretur_pembelian_header h $where_base");
        $tm_tot   = mysqli_fetch_array($cari_tot);
        $tot = $tm_tot['tot'];
        $hasil_cari = "Ditemukan ".$tot." data";

        // Daftar supplier untuk filter
        ...
        if(isset($_POST['btnrst'])) {
            ...
            if($cbo_supplier != '') {
                $where .= " AND ph.no_supplier='$cbo_supplier'";
            }
            $sql_query = "SELECT h.noretur, h.nopembelian, DATE_FORMAT(h.tanggal,'%d/%m/%Y') AS tanggal_trx,
                                 h.total_qty_retur, h.total_akhir, h.status_retur, h.note,
                                 COALESCE(s.namasupplier,'') AS namasupplier
                        ...
                        <div class="col-xs-12">
                            <div class="widget-box">
                                <div class="widget-header">
                                    <h4 class="widget-title"><i class="fa fa-undo"></i> Laporan Retur Pembelian</h4>
                                </div>
                                    ...
                                    <div class="widget-main">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-xs-12 col-sm-3">
                                                ...
                                                <select class="form-control chosen-select" name="cbosupplier" id="cbosupplier">
                                                    <option value="">-- Semua Supplier --</option>
                                                    <?php
                                                    mysqli_data_seek($sql_sup, 0);
                                                    while($rs = mysqli_fetch_assoc($sql_sup)): ?>
                                                    <option value="<?php echo htmlspecialchars($rs['nosupplier']); ?>"
                                                        <?php echo ($cbo_supplier==$rs['nosupplier'])?'selected':''; ?>>
                                                        <?php echo htmlspecialchars($rs['namasupplier']); ?>
                                                    </option>
                                                    <?php endwhile; ?>
                                                </select>
                                            </div>
                                        ...
                                        <table class="table table-bordered table-striped table-condensed">
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo htmlspecialchars($row['nopembelian']); ?></td>
                                                ...
                                                <td class="center">
                                                    <a href="retur_pembelian_detail.php?noretur=<?php echo urlencode($row['noretur']); ?>" class="btn btn-xs btn-info">
                                                        <i class="fa fa-eye"></i> Detail
                                                    </a>
                                                    <a href="retur_pembelian_cetak.php?noretur=<?php echo urlencode($row['noretur']); ?>" class="btn btn-xs btn-default" target="_blank">

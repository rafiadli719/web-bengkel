...
if(empty($_SESSION['_iduser'])){
    ...
    $allowed_cols = ['noretur','nopenjualan','namapelanggan'];
    $where_key = "";
    if($txtkey != '') {
        if($cbocari != '' && in_array($cbocari, $allowed_cols)) {
            $where_key = "AND $cbocari LIKE '%$txtkey%'";
        } else {
            $where_key = "AND (noretur LIKE '%$txtkey%' OR nopenjualan LIKE '%$txtkey%' OR namapelanggan LIKE '%$txtkey%')";
        }
    }

    $sql_query = "SELECT *, DATE_FORMAT(tgl_retur,'%d/%m/%Y') AS tanggal_retur_fmt,
                         DATE_FORMAT(tgl_jual,'%d/%m/%Y') AS tanggal_jual_fmt
                  FROM view_retur_penjualan
                  WHERE kd_cabang='$kd_cabang' $where_key
                  ORDER BY tgl_retur DESC";

    $res_count = mysqli_query($koneksi, "SELECT count(*) as tot FROM view_retur_penjualan WHERE kd_cabang='$kd_cabang' $where_key");
    $row_count = mysqli_fetch_array($res_count);
    $tot = $row_count['tot'];
    $hasil_cari = "Ditemukan <strong>$tot</strong> data retur penjualan";
?>
<!DOCTYPE html>
<html lang="en">
<head>
                    ...
                    <div class="row">
                        <div class="col-xs-12 col-sm-4">
                            <a href="retur_penjualan_add.php">
                                <button class="btn btn-success btn-block"><i class="fa fa-plus"></i> Input Retur Penjualan</button>
                            </a>
                        </div>
                        ...
                        <div class="col-xs-12">
                            ...
                            <table class="table table-bordered table-hover table-striped" id="dynamic-table">
                                ...
                                while ($tampil = mysqli_fetch_array($sql)) {
                                    $hdr = mysqli_query($koneksi, "SELECT total_akhir, total_qty_retur FROM tblretur_penjualan_header WHERE noretur='".mysqli_real_escape_string($koneksi,$tampil['noretur'])."'");
                                ...
                                <tr>
                                    <td class="center">
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu dropdown-default">
                                                <li><a href="retur_penjualan_detail.php?noretur=<?php echo urlencode($tampil['noretur']); ?>">
                                                    <i class="fa fa-eye"></i> Detail</a></li>
                                                <li><a href="retur_penjualan_cetak.php?noretur=<?php echo urlencode($tampil['noretur']); ?>" target="_blank">
                                                    <i class="fa fa-print"></i> Cetak</a></li>
                                                <?php if($status_retur == '0'): ?>
                                                <li class="divider"></li>
                                                <li><a href="retur_penjualan_batal.php?noretur=<?php echo urlencode($tampil['noretur']); ?>"
                                    ...
                                    <td><?php echo htmlspecialchars($tampil['nopenjualan']); ?></td>
                                    <td class="center"><?php echo htmlspecialchars($tampil['tanggal_jual_fmt']); ?></td>
                                    <td><?php echo htmlspecialchars($tampil['namapelanggan']); ?></td>

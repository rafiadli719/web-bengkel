...
if(empty($_SESSION['_iduser'])){
    ...
    $init_noservice = isset($_GET['no_service']) ? htmlspecialchars($_GET['no_service']) : '';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="no-skin">
            ...
            <div class="main-content-inner">
                <div class="breadcrumbs ace-save-state" id="breadcrumbs">
                    <ul class="breadcrumb">
                        ...
                        <li><a href="retur_servis.php">Retur Servis</a></li>
                        ...
                        <div class="col-xs-12">
                            <div class="widget-box">
                                <div class="widget-header">
                                    <h4 class="widget-title"><i class="fa fa-undo"></i> Input Retur Servis</h4>
                                </div>
                                <div class="widget-body">
                                    <div class="widget-main">
                                        ...
                                        <div class="row">
                                            <div class="col-xs-12 col-sm-8">
                                                <div class="input-group">
                                                    <span class="input-group-addon"><b>No. Service</b></span>
                                                    <input type="text" id="txt_noservice" class="form-control"
                                                           placeholder="Masukkan nomor service (contoh: SV26000001828)"
                                                           value="<?php echo $init_noservice; ?>" />
                                        ...
                                        <div id="info-servis" class="alert alert-info">
                                            <strong>No. Service:</strong> <span id="lbl_noservice"></span> &nbsp;|&nbsp;
                                            <strong>Pelanggan:</strong> <span id="lbl_pelanggan"></span> &nbsp;|&nbsp;
                                            <strong>Cara Bayar:</strong> <span id="lbl_carabayar"></span>
                                        </div>

                                        <form id="form-retur" action="save_retur_servis.php" method="post">
                                            <input type="hidden" name="no_service" id="hid_noservice" />
                                                        ...
                                                        <table class="table table-bordered tbl-item">
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th class="center" width="7%">Qty Asal</th>
                                                ...
                                                <div class="row">
                                                    <div class="col-xs-12 col-sm-5">
                                                        <div class="form-group">
                                                            ...
                                                            <input type="text" name="note" class="form-control" placeholder="Opsional" maxlength="100" />
                                                        </div>
                                                        <div class="form-group">
                                                ...
                                                <div class="row">
                                                    <div class="col-xs-12">
                                                        ...
                                                        <button type="submit" class="btn btn-danger btn-lg">
                                                            <i class="fa fa-save"></i> Simpan Retur Servis
                                                        </button>
                                                        &nbsp;
                                                        <a href="retur_servis.php" class="btn btn-default btn-lg">
    ...
    $('#btn_load_items').on('click', function(){
        var no_service=$.trim($('#txt_noservice').val());
        if(no_service==''){$('#msg_error').text('No. Service tidak boleh kosong').show();return;}
        $('#msg_error').hide();
        var self=this; $(self).prop('disabled',true).html('<i class="fa fa-spinner fa-spin"></i> Loading...');
        $.getJSON('ajax_get_items_retur_servis.php',{no_service:no_service},function(data){
            $(self).prop('disabled',false).html('<i class="fa fa-search"></i> Load Items');
            if(data.error){$('#msg_error').text(data.error).show();$('#info-servis').hide();$('#tbl-items-container').hide();return;}
            $('#lbl_noservice').text(data.no_service); $('#lbl_pelanggan').text(data.namapelanggan); $('#lbl_carabayar').text(data.carabayar || '-');
            $('#hid_noservice').val(data.no_service); $('#info-servis').show();
            ...
            for(var i=0;i<data.items.length;i++){
                ...
                html+='<td>'+(it.tipe_item=='barang'?'Barang':'Jasa')+'<input type="hidden" name="item_tipe[]" value="'+it.tipe_item+'"/></td>';
                html+='<td>'+it.no_item+'<input type="hidden" name="item_kode[]" value="'+it.no_item+'"/></td>';
                html+='<td>'+it.namaitem+'</td><td class="center">'+it.qty_asal+'</td><td class="center">'+it.qty_retur+'</td>';
                ...
                html+='<td class="harga-cell">'+formatRibuan(it.harga)+'<input type="hidden" class="harga-data" name="item_harga[]" value="'+it.harga+'"/></td>';
                html+='<td class="harga-cell subtotal-cell">0</td>';
                html+='<td><input type="text" class="form-control input-sm" name="item_alasan[]" placeholder="Alasan..." maxlength="100"/></td></tr>';
            }
    ...
    $('#form-retur').on('submit',function(e){
        if($('#hid_noservice').val()==''){e.preventDefault();alert('Load items service terlebih dahulu.');return;}
        ...
        $('.chk-item').each(function(){
            ...
            row.find('input[name="item_qty[]"],input[name="item_kode[]"],input[name="item_tipe[]"],input[name="item_harga[]"],input[name="item_alasan[]"]').prop('disabled',!enabled);
        });
        if(!confirm('Simpan retur servis ini?')){
            e.preventDefault();
            $('[name="item_qty[]"],[name="item_kode[]"],[name="item_tipe[]"],[name="item_harga[]"],[name="item_alasan[]"]').prop('disabled',false);
        }
    });
    <?php if($init_noservice): ?>

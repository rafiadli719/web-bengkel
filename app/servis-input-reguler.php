    ...
    // --------------------
        ...
        if(isset($_POST['btnbayar'])) {
            ...
            if (function_exists('checkDiskonApproval') && !checkDiskonApproval($koneksi, $no_service, $txtpotfaktur_persen, $txtpotfaktur_nom, $id_user, $kd_cabang)) {
                echo "<script>window.alert('Diskon manual di luar SOP butuh approval Supervisor/Manager sebelum pembayaran bisa diproses. Request approval sudah dikirim.'); window.location='servis-input-reguler.php?snoserv=" . urlencode($no_service) . "&tab=actions';</script>";
                exit;
            }
            ...
            if (isset($_POST['btnaddkeluhan'])) {
                ...
                if (!empty($txtkeluhan)) {
                    ...
                    echo "<script>
                        ...
                        window.location.href = 'servis-input-reguler.php?snoserv=$no_service_post&tab=details';
                    </script>";
                    exit;
                } else {
                    echo "<script>
                                ...
                                while($pending_data = mysqli_fetch_assoc($q_pending_all)) {
                                    if($pending_data['tipe'] == 'barang') {
                                        ...
                                        if($c == 0) {
                                            ...
                                            mysqli_query($koneksi, "INSERT INTO tblservis_barang (no_service, kd_cabang, nobaris, no_item, quantity, qty_retur, harga_jual, potongan, total, diskon_source, diskon_persen, diskon_nominal, id_promo) VALUES ('$no_service_post', '".mysqli_real_escape_string($koneksi, $kd_cabang)."', '".$nb."', '".$pending_data['kode_item']."', '".$qty."', '0', '".$harga."', '".$diskon_persen."', '".$subtotal."', '".$diskon_source."', '".$diskon_persen."', '".$diskon_nominal."', $id_promo)");
                                        ...
                                        if($c == 0) {
                                            ...
                                            mysqli_query($koneksi, "INSERT INTO tblservis_jasa (no_service, kd_cabang, nobaris, no_item, waktu, harga, potongan, total, diskon_source, diskon_persen, diskon_nominal, id_promo) VALUES ('$no_service_post', '".mysqli_real_escape_string($koneksi, $kd_cabang)."', '".$nb."', '".$pending_data['kode_item']."', '".$waktu."', '".$harga."', '".$diskon_persen."', '".$subtotal."', '".$diskon_source."', '".$diskon_persen."', '".$diskon_nominal."', $id_promo)");
            ...
            // Tampilkan notifikasi peringatan
             echo "<script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    var container = document.querySelector('.ks-left') || document.querySelector('.page-content') || document.body;
                    if (!container) {
                        return;
                    }
...
<div class="ks-shell">
    ...
    <div class="ks-topbar">
        ...
        <div class="ks-topbar-right">
            ...
            <a href="servis-reguler.php" class="ks-topbar-btn">
        ...
        <div class="ks-body">
            ...
            <div class="ks-center">
                ...
                <div class="ks-tabs-nav">
                    <a class="ks-tab-btn <?= $active_tab=='workorder-details'?'active':'' ?>"
                       data-target="workorder-details" href="<?= $tab_base_url ?>&tab=workorder">
                        ...
                        <?php $count_wo=0; $r=mysqli_query($koneksi,"SELECT COUNT(*) c FROM tbservis_workorder WHERE no_service='$no_service'"); if($r){$count_wo=mysqli_fetch_assoc($r)['c'];} if($count_wo>0): ?>
                        <span class="ks-badge"><?= $count_wo ?></span><?php endif; ?>
                    </a>
                ...
                <div class="ks-tab-contents">
                    <?php
                    // Default active tab: workorder jika tidak dikenali
                    $center_active = in_array($active_tab, ['workorder-details','temuan-penawaran','service-items','service-jasa'])
                                     ? $active_tab : 'workorder-details';
                    ?>
    ...
    (function() {
        ...
        function initServiceTabs() {
            document.querySelectorAll('.ks-tab-btn').forEach(function(button) {
            ...
            var activePane = document.querySelector('.ks-tab-pane.active');
            var defaultTarget = activePane ? activePane.id : 'workorder-details';
            window.switchServiceTab(defaultTarget);

            document.querySelectorAll('form').forEach(function(form) {
                form.addEventListener('submit', function() {
                    var activeButton = document.querySelector('.ks-tab-btn.active');
                    var activeTarget = activeButton ? activeButton.getAttribute('data-target') : 'workorder-details';
        ...
        window.switchServiceTab = function(target) {
            document.querySelectorAll('.ks-tab-btn').forEach(function(btn) {
                btn.classList.toggle('active', btn.getAttribute('data-target') === target);
            });

            document.querySelectorAll('.ks-tab-pane').forEach(function(pane) {

...
if(empty($_SESSION['_iduser'])){
    ...
    while($detail = mysqli_fetch_array($wo_details)) {
        if($detail['tipe'] == '1') { // Jasa
            mysqli_query($koneksi,"DELETE FROM tblservis_jasa WHERE no_service='$snoserv' AND no_item='{$detail['kode_barang']}' AND kd_cabang='$kd_cabang' LIMIT 1");
        } else { // Barang
            mysqli_query($koneksi,"DELETE FROM tblservis_barang WHERE no_service='$snoserv' AND no_item='{$detail['kode_barang']}' AND kd_cabang='$kd_cabang' LIMIT 1");
        }
    }

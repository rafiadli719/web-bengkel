<?php
    ...
    if(isset($_POST['btnadd'])) {
        ...
        if(!empty($txtkdbarang) && $txtqty > 0) {
            ...
            if($harga_data = mysqli_fetch_array($cari_harga)) {
                ...
                if($diskon_source === 'promo' && intval($id_promo) > 0 && function_exists('logPromoUsage')) { logPromoUsage($koneksi, $id_promo, $no_service, 'barang', $txtkdbarang, $diskon_nominal); }
    ...
    if (isset($_POST['btnadd_partcust'])) {
        $no_service_pc = mysqli_real_escape_string($koneksi, $_POST['txtnosrv'] ?? '');
        if (!empty($no_service_pc)) {
    ...
    if(isset($_POST['btnaddsrv'])) {
        ...
        if(!empty($txtcarisrv)) {
            ...
            if($wo_data = mysqli_fetch_array($cari_wo)) {
                ...
                if($diskon_source === 'promo' && intval($id_promo) > 0 && function_exists('logPromoUsage')) { logPromoUsage($koneksi, $id_promo, $no_service, 'jasa', $txtcarisrv, $diskon_nominal); }
    ...
    if(isset($_POST['btnaddworkorder'])) {
        ...
        // Kumpulkan ke array PHP, expand baris tipe=3 (kombinasi WO) jadi baris jasa/barang milik WO anak.
        $detail_rows = array();
        if ($detail_wo) {
            ...
            while($pending_data = mysqli_fetch_assoc($q_pending_all)) {
                if($pending_data['tipe'] == 'barang') {
                    ...
                    if($c == 0) {
                        ...
                        $q_promo_wo = mysqli_query($koneksi, "SELECT id_promo, tipe_promo, nilai_promo FROM v_promo_target_legacy WHERE target_type='workorder' AND ($wo_condition_sql) AND status_aktif=1 AND '".$tgl_cek."' BETWEEN tanggal_mulai AND tanggal_selesai ORDER BY nilai_promo DESC LIMIT 1");
                        ...
                        if($diskon_source === 'promo' && $id_promo !== 'NULL' && function_exists('logPromoUsage')) { logPromoUsage($koneksi, $id_promo, $no_service, 'barang', $pending_data['kode_item'], $diskon_nominal); }
                        $auto_barang++;
                    }
                } else { // jasa
                    // Cek duplikat
                    ...
                    if($c == 0) {
                        ...
                        if($diskon_source === 'promo' && $id_promo !== 'NULL' && function_exists('logPromoUsage')) { logPromoUsage($koneksi, $id_promo, $no_service, 'jasa', $pending_data['kode_item'], $diskon_nominal); }
                        $auto_jasa++;
                    }
                }

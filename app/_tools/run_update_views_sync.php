...
$sqlFile = $baseDir . '/tools/sql/update_views_customer_vehicle_sync.sql';
if (!is_file($sqlFile)) {
...
if ($confirm !== '1') {
    echo '<h3>Update VIEW Customer/Vehicle Sync</h3>';
    echo '<p>Tool ini akan menjalankan semua statement di <code>' . h($sqlFile) . '</code>:</p>';
    echo '<ul>';
    echo '<li><code>view_cari_pelanggan</code> &mdash; tambah kolom status_member + diskon_persen dari statistik_pelanggan</li>';
    echo '<li><code>view_cari_kendaraan</code> &mdash; pemilik/telepon diambil dari pelanggan hasil sync (mapping servis terakhir)</li>';
    echo '<li><code>view_pelanggan_kendaraan</code> &mdash; sama, dipakai popup cari kendaraan</li>';
    echo '</ul>';
...
echo '<h3>Hasil eksekusi update_views_customer_vehicle_sync.sql</h3>';

<?php
    ...
    if ($row && in_array($row['status_servis'], ['bayar', 'selesai'])) {
        echo "<script>alert('Jasa tidak bisa dihapus — servis sudah " . strtoupper($row['status_servis']) . ".');window.location=('servis-input-reguler-rst.php?snoserv=$no_service');</script>";
        exit;
    }

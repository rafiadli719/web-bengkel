...
// ===================
	...
	$cari_kd=mysqli_query($koneksi,"SELECT 
									DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal_trx, no_pelanggan, 
	...
	$query = mysqli_query($koneksi,"SELECT 
                                    no_penjualan, keterangan, jumlah_piutang, jumlah_bayar, status 
                                    FROM 
                                    tblpiutang_detail 
                                    WHERE no_transaksi='$no_bukti'");
...
'.nota_pdf_header($file_logo, $nama_perusahaan, $alamat, $notlp, $fax, 'BUKTI PEMBAYARAN PIUTANG', array(
            array('No. Trs', $no_bukti),
            array('Tanggal', $tgltrx_eng),
            array('Pelanggan', $no_supplier.'&nbsp;'.$namasupplier),
            array('User', $user),
        )).'
        ...
        $html .= "<tr>
                ...
                <td><font size=2>".$row['no_penjualan']."</font></td>

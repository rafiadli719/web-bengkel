<!-- Tab Actions Jemput RST - Enhanced with Payment -->
<style>
.has-error {
            ...
            <div class="padding-18">
                    ...
                    <div class="col-sm-12">
                        <div class="alert alert-info">
                            ...
                            <?php if($kategori_pelanggan && $kategori_pelanggan != '001'): ?>
                                ...
                                ?>
                            <?php endif; ?>
                            | <i class="ace-icon fa fa-truck"></i> <strong>Service Jemput RST</strong>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        ...
                        (function() {
                            function toggleBuktiPembayaranJemput() {
                            ...
                            if (document.readyState === 'loading') {
                                document.addEventListener('DOMContentLoaded', toggleBuktiPembayaranJemput);
                            } else {
                                toggleBuktiPembayaranJemput();
                            }
                            
                            // Add event listener
                            var metodeBayar = document.getElementById('metode_pembayaran');
                            if (metodeBayar) {
                                metodeBayar.addEventListener('change', toggleBuktiPembayaranJemput);
                            }
                        })();
                        </script>
                        ...
                        <!-- Jemput RST Specific Info -->
                        <div class="alert alert-warning">
                            <i class="ace-icon fa fa-exclamation-triangle"></i>
                            <strong>Info Jemput RST:</strong> Total dapat berubah setelah analisis kerusakan. Kendaraan akan diantar kembali setelah selesai.
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-sm-6">
                            ...
                            <div class="col-sm-5">
                                <select name="cbokepala_mekanik1" id="cbokepala_mekanik1" class="form-control">
                                    ...
                                    $q_km1 = "SELECT id, kode_karyawan, COALESCE(nama_lengkap, nama_user) AS nama
                                              FROM tbuser
                                              WHERE (status_row='0' OR status_row IS NULL) AND (is_active='active' OR is_active IS NULL)
                                                AND ((TRIM(UPPER(kode_posisi))='KM' OR UPPER(kode_posisi) LIKE '%KEPALA%MEKANIK%') OR user_akses=10)
                                                AND (TRIM(UPPER(kode_cabang)) = TRIM(UPPER('".mysqli_real_escape_string($koneksi, $kd_cabang)."')) OR IFNULL(kode_cabang,'')='' OR TRIM(UPPER(kode_cabang)) IN ('ALL','SEMUA'))
                                              ORDER BY nama";
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <select name="cbomekanik1" id="cbomekanik1" class="form-control" data-required="true">
                                    <option value="">- Pilih Mekanik -</option>
                                    <?php
                                    $q_mk1 = "SELECT id, kode_karyawan, COALESCE(nama_lengkap, nama_user) AS nama
                                              FROM tbuser
                                              WHERE (status_row='0' OR status_row IS NULL) AND (is_active='active' OR is_active IS NULL)
                                                AND (TRIM(UPPER(kode_posisi))='MK' OR UPPER(kode_posisi) LIKE '%MEKANIK%')
                                                AND (TRIM(UPPER(kode_cabang)) = TRIM(UPPER('".mysqli_real_escape_string($koneksi, $kd_cabang)."')) OR IFNULL(kode_cabang,'')='' OR TRIM(UPPER(kode_cabang)) IN ('ALL','SEMUA'))
                                              ORDER BY nama";
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <select name="cboadmin1" id="cboadmin1" class="form-control">
                                    ...
                                    <?php
                                    $q_admin1 = "SELECT id, kode_karyawan, COALESCE(nama_lengkap, nama_user) AS nama
                                                FROM tbuser
                                                WHERE status_row='0' AND is_active='active'
                                                  AND (kode_posisi IN ('ADM','CS') OR user_akses IN (1,2,4))
                                                  AND (kode_cabang='".mysqli_real_escape_string($koneksi, $kd_cabang)."' OR IFNULL(kode_cabang,'')='')
                                                ORDER BY nama";
                        ...
                        <!-- Admin 2 -->
                        <div class="form-group">
                            <label class="col-sm-4 control-label no-padding-right">Admin 2:</label>
                            <div class="col-sm-5">
                                <select name="cboadmin2" id="cboadmin2" class="form-control">
                                    <option value="">- Pilih Admin -</option>
                                    <?php
                                    $q_admin2 = "SELECT id, kode_karyawan, COALESCE(nama_lengkap, nama_user) AS nama
                                                FROM tbuser
                                                WHERE status_row='0' AND is_active='active'
                                                  AND (kode_posisi IN ('ADM','CS') OR user_akses IN (1,2,4))
                                                  AND (kode_cabang='".mysqli_real_escape_string($koneksi, $kd_cabang)."' OR IFNULL(kode_cabang,'')='')
                ...
                <div class="row">
                    ...
                    <div class="col-xs-2">
                        ...
                        id="btnupdatemekanik" name="btnupdatemekanik">
                            <i class="ace-icon fa fa-users"></i>
                            Update Mekanik
                        </button>
                    </div>
...
(function() {
    if (typeof jQuery === 'undefined') {
        document.addEventListener('DOMContentLoaded', initServiceActionsJemputRst);
    } else {
        jQuery(document).ready(initServiceActionsJemputRst);
    }

    function initServiceActionsJemputRst() {
        var $ = jQuery;
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success && response.has_data) {
                        ...
                        // Kepala Mekanik 1: match by text/value (case-insensitive); append if not found
                        if (data.kepala_mekanik_1) {
                            ...
                            $('#cbokepala_mekanik1 option').each(function(){
                                ...
                                if(t===name1 || v===name1){ $(this).prop('selected',true).trigger('change'); filled=true; found1=true; return false; }
                            });
                            if(!found1){ var $s1=$('#cbokepala_mekanik1'); $s1.append('<option value="'+name1Raw+'">'+name1Raw+'</option>'); $s1.val(name1Raw).trigger('change'); filled=true; }
                        }

                        // Kepala Mekanik 2: match by text/value; append if not found
                        if (data.kepala_mekanik_2) {
                            ...
                            $('#cbokepala_mekanik2 option').each(function(){
                                var t=$(this).text().trim().toLowerCase();
                                var v=String($(this).val()||'').trim().toLowerCase();
                                if(t===name2 || v===name2){ $(this).prop('selected',true).trigger('change'); filled=true; found2=true; return false; }
                            });
                            if(!found2){ var $s2=$('#cbokepala_mekanik2'); $s2.append('<option value="'+name2Raw+'">'+name2Raw+'</option>'); $s2.val(name2Raw).trigger('change'); filled=true; }
                        }

                        // Update total percentage if function exists
                        if (typeof window.updateTotalPercentage === 'function') {
                            window.updateTotalPercentage();
                        }
                        if (autoSave === true && typeof window.saveKepalaMetanikToService === 'function') {
                            window.saveKepalaMetanikToService();
                        }
                        alert('Kepala mekanik berhasil di-auto fill dari data harian!');

                    } else {
                        alert(response.message || 'Belum ada data kepala mekanik untuk hari ini');
                ...
                error: function() {
                    alert('Gagal mengambil data kepala mekanik harian');
                }
            });
        };

        // ============ SAVE KEPALA MEKANIK KE SERVICE ============
        window.saveKepalaMetanikToService = function(){
            ...
            $.ajax({
                ...
                success:function(r){ if(r && r.success){ console.log('Kepala mekanik disimpan'); } },
                error:function(){ console.log('Gagal simpan kepala mekanik'); }
            });
        };
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        alert('Data kepala mekanik harian berhasil direfresh!');
                    } else {
                        alert(response.message || 'Belum ada data kepala mekanik untuk hari ini');
                    }
                },
                error: function() {
                    alert('Gagal refresh data kepala mekanik harian');
                }
            });
        };

        // ============ FORM VALIDATION ============
        // Custom validation for required fields in tabs
        function validateRequiredFields() {
            var isValid = true;
            var firstInvalidField = null;
            
            // Check all fields with data-required="true"
            ...
            // If validation fails, show the tab containing the first invalid field
            if (!isValid && firstInvalidField) {
                ...
                if ($tabPane.length > 0) {
                    ...
                    // Focus on the invalid field after tab is shown
                    setTimeout(function() {
        ...
        // Override form submission to include custom validation
        window.validateFormBeforeSubmit = function(formAction) {
            if (formAction === 'btnbayar' || formAction === 'btnupdatemekanik') {
                return validateRequiredFields();
            }
            return true; // Allow other actions to proceed without validation
        };
        
        // Add event listeners to form buttons
        $('#btnbayar, #btnupdatemekanik').on('click', function(e) {
            var buttonName = $(this).attr('name');
            if (!validateFormBeforeSubmit(buttonName)) {
                e.preventDefault();
                return false;
            }
        });
        ...
        $('[data-required="true"]').on('change blur', function() {
            var $field = $(this);
            var value = $field.val();
        ...
        // ============ MECHANIC PERCENTAGE CALCULATION ============
        function updateTotalPercentage() {
            var total = 0;
            
            // Sum all percentage fields
            $('[id*="txtpersen_"]').each(function() {
                var value = parseFloat($(this).val()) || 0;
                total += value;
            });
            
            // Update display
            var $display = $('#totalPercentageDisplay');
            if ($display.length > 0) {
                var statusClass = total === 100 ? 'text-success' : (total > 100 ? 'text-danger' : 'text-warning');
                $display.html('<strong class="' + statusClass + '">Total: ' + total + '%</strong>');
            }
            
            return total;
        }
        
        // Auto calculate percentage when mechanic is selected
        window.autoCalculatePercentage = function() {
            var selectedMechanics = [];
            
            // Count selected mechanics (excluding empty values)
            $('select[id*="mekanik"], select[id*="kepala_mekanik"], select[id*="admin"]').each(function() {
                if ($(this).val() && $(this).val() !== '') {
                    selectedMechanics.push($(this));
                }
            });
            
            if (selectedMechanics.length === 0) {
                alert('Pilih minimal satu mekanik terlebih dahulu');
                return;
            }
            
            // Distribute percentage equally
            var percentagePerMechanic = Math.floor(100 / selectedMechanics.length);
            var remainder = 100 % selectedMechanics.length;
            
            selectedMechanics.forEach(function($select, index) {
                var mechanicId = $select.attr('id');
                var percentageFieldId = mechanicId.replace('cbo', 'txtpersen_');
                var $percentageField = $('#' + percentageFieldId);
                
                if ($percentageField.length > 0) {
                    var percentage = percentagePerMechanic;
                    if (index < remainder) {
                        percentage += 1; // Distribute remainder to first few mechanics
                    }
            ...
            alert('Persentase berhasil dihitung otomatis: ' + selectedMechanics.length + ' mekanik terpilih');
        };
        ...
        window.saveMechanicData = function() {
            ...
            if (total !== 100) {
                if (!confirm('Total persentase adalah ' + total + '%, bukan 100%. Lanjutkan menyimpan?')) {
                    return false;
                }
            }
            
            // Trigger the update mechanic button
            $('#btnupdatemekanik').click();
        };
        
        // Update percentage display when values change
        $('[id*="txtpersen_"]').on('input change', function() {
            updateTotalPercentage();
        });
        
        // Auto-calculate when mechanic selection changes
        $('select[id*="mekanik"], select[id*="kepala_mekanik"], select[id*="admin"]').on('change', function() {
            // Optional: Auto-calculate when selection changes

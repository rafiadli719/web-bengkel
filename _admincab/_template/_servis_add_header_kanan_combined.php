<?php
// File: _servis_add_header_kanan_combined.php  
...
$persen_kepala1 = 0;
$persen_kepala2 = 0;
// Admin fields mapped to mekanik1/2 in database
$admin1 = '';
$admin2 = '';
$persen_admin1 = 0;
$persen_admin2 = 0;
...
if(isset($no_service) && isset($koneksi)) {
    try {
        ...
        if($cari_data && $tm_data = mysqli_fetch_array($cari_data)) {
            ...
            $persen_kepala1 = $tm_data['persen_kepala_mekanik1'] ?? 0;
            $persen_kepala2 = $tm_data['persen_kepala_mekanik2'] ?? 0;
            
            // Map admin fields to mekanik1/2 for display (admin1->mekanik1, admin2->mekanik2)
            $admin1 = $tm_data['mekanik1'] ?? '';
            $admin2 = $tm_data['mekanik2'] ?? '';
            $persen_admin1 = $tm_data['persen_mekanik1'] ?? 0;
            $persen_admin2 = $tm_data['persen_mekanik2'] ?? 0;
            
            // Actual mekanik fields (mekanik1-4 in form = mekanik1-4 in DB)
            $mekanik1_val = $tm_data['mekanik1'] ?? '';
            $mekanik2_val = $tm_data['mekanik2'] ?? '';
            $mekanik3_val = $tm_data['mekanik3'] ?? '';
            $mekanik4_val = $tm_data['mekanik4'] ?? '';
            $persen1 = $tm_data['persen_mekanik1'] ?? 0;
            $persen2 = $tm_data['persen_mekanik2'] ?? 0;
            $persen3 = $tm_data['persen_mekanik3'] ?? 0;
            $persen4 = $tm_data['persen_mekanik4'] ?? 0;
        ...
        <div class="widget-main">
                                        ...
                                        <?php 
                                            ...
                                            if ($sql_keluhan_list) {
                                                while ($tampil = mysqli_fetch_array($sql_keluhan_list)) {
                                                    ...
                                                    echo "<button type='button' class='btn btn-xs btn-danger' onclick='hapusKeluhan(" . (int)$tampil['id'] . ")' title='Hapus Keluhan'><i class=\"ace-icon fa fa-trash\"></i></button>";
                                                    echo "</div>";
                                                    echo "</div>";
                                                    echo "</li>";
                                                    $counter++;
                                            ...
                                            if ($sql_wo_list) {
                                                while ($tampil = mysqli_fetch_array($sql_wo_list)) {
                                                    echo "<li style='margin-bottom: 10px; padding: 8px; border-left: 4px solid #3498db; background-color: #ebf3fd; font-size: 14px; position: relative;'>";
                                                    echo "<div style='display: flex; justify-content: space-between; align-items: center;'>";
                                                    echo "<span style='flex: 1; font-weight: bold; color: #333;'>";
                                                    ...
                                                    echo "</span>";
                                                    echo "<div class='spk-actions' style='margin-left: 10px;'>";
                                                    echo "<button type='button' class='btn btn-xs btn-danger' onclick='hapusWorkOrder(" . (int)$tampil['id'] . ")' title='Hapus Work Order'><i class=\"ace-icon fa fa-trash\"></i></button>";
                                                    echo "</div>";
                                                    echo "</div>";
                                                    echo "</li>";
                                                    $counter++;
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12">
                    ...
                    <div class="alert alert-info" style="margin-bottom: 15px; padding: 10px;">
                        <i class="ace-icon fa fa-info-circle"></i>
                        <strong>Info:</strong> Pengisian persentase <strong>OPSIONAL</strong> saat input SPK.
                        Persentase dapat diisi sebelum pembayaran/penyelesaian service.
                    </div>
                </div>

                <!-- Kepala Mekanik 1 dengan Persentase -->
                <div class="col-xs-8 col-sm-8">
                    <label>Kepala Mekanik 1 (Opsional):</label>
                    <select class="form-control" name="cbokepala_mekanik1" id="cbokepala_mekanik1" onchange="autoFillKepalaPercentage()">
                ...
                <div class="col-xs-8 col-sm-8">
                    ...
                    <select class="form-control" name="cbokepala_mekanik2" id="cbokepala_mekanik2" onchange="autoFillKepalaPercentage()">
                        <option value="">- Pilih Kepala Mekanik -</option>
                        <?php
                            if(isset($koneksi)) {
                                try {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12">
                    ...
                    <p><small><em>Pengisian admin/kasir dan persentase OPSIONAL. Dapat diisi sebelum pembayaran.</em></small></p>
                </div>

                <!-- Admin/Kasir 1 with percentage -->
                <div class="col-xs-8 col-sm-8">
                    <label>Admin/Kasir 1 (Opsional):</label>
                    <select name="cboadmin1" id="cboadmin1" class="form-control" onchange="autoFillAdminPercentage()">
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12">
                    ...
                    <p><small><em>Pengisian mekanik dan persentase OPSIONAL. Dapat diisi sebelum pembayaran.</em></small></p>
                </div>

                <!-- Mekanik 1 with percentage -->
                <div class="col-xs-8 col-sm-8">
                    <label>Mekanik 1 (Opsional):</label>
                    <select name="cbomekanik1" id="cbomekanik1" class="form-control" onchange="autoFillMekanikPercentage()">
            ...
            <!-- Debug Auto-Save Status Panel - HIDDEN -->
            <div class="row" style="display: none;">
                <div class="col-xs-12 col-sm-12">
                    <div class="alert alert-info" style="padding: 10px;">
                        <h6 class="header blue smaller" style="margin: 0 0 10px 0;">
                            <i class="ace-icon fa fa-bug"></i> Debug Auto-Save Status
                        </h6>
...
function validateMekanikKepala() {
    // Validation is now optional - removed required validation
    // Kepala mekanik can be filled before payment/completion
    return true;
}
...
function validateAdmin() {
    // Validation is now optional - removed required validation
    // Admin can be filled before payment/completion
    return true;
}
...
function validateMekanik() {
    // Validation is now optional - removed required validation
    // Mekanik can be filled before payment/completion
    return true;
}
...
function showSaveIndicator(fieldType, status) {
    ...
    var indicatorId = 'save-indicator-' + fieldType.replace(/[^a-zA-Z0-9]/g, '');
    var $indicator = $('#' + indicatorId);
    
    if ($indicator.length === 0) {
        $indicator = $('<span id="' + indicatorId + '" class="save-indicator" style="margin-left: 5px; font-size: 12px;"></span>');
        
        // Find appropriate parent element based on field type
        if (fieldType.includes('kepala')) {
            $('#txtpersen_kepala1, #txtpersen_kepala2').first().parent().append($indicator);
        } else if (fieldType.includes('admin')) {
            $('#txtpersen_admin1, #txtpersen_admin2').first().parent().append($indicator);
        } else if (fieldType.includes('mekanik')) {
            $('#txtpersen_kerja1, #txtpersen_kerja2, #txtpersen_kerja3, #txtpersen_kerja4').first().parent().append($indicator);
        } else if (fieldType === 'km') {
            $('#txtkm_skr').parent().append($indicator);
        }
    }
    
    // Update indicator
    if (status === 'success') {
        $indicator.html('<i class="ace-icon fa fa-check text-success"></i>').show();
    } else {
        $indicator.html('<i class="ace-icon fa fa-times text-danger"></i>').show();
    }
...
function forceSaveAllData() {
    ...
    addDebugMessage('Force saving all data...', 'process');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    addDebugMessage('Save failed: ' + fieldType + ' - ' + response.message, 'error');
                    reject(response);
                }
            },
            error: function(xhr, status, error) {
...
$(document).ready(function() {
    ...
    setInterval(function() {
        addDebugMessage('Auto-save interval triggered', 'info');
        autoSaveAllData();
    }, 3000);
    ...
    // Auto-calculate when values change for Kepala Mekanik
    $('#txtpersen_kepala1, #txtpersen_kepala2').on('keyup change', function() {
        calculatePercentageKepala();
    });
    
    // Auto-calculate when values change for Admin
    $('#txtpersen_admin1, #txtpersen_admin2').on('keyup change', function() {
        calculatePercentageAdmin();
    });
    
    // Auto-calculate when values change for Mekanik
    ...
    // Auto-save Kepala Mekanik
    $('#cbokepala_mekanik1').on('change', function() {
        var fieldValue = $(this).val();
        var fieldPercentage = $('#txtpersen_kepala1').val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('kepala_mekanik1', fieldValue, fieldPercentage);
        }
    });

    $('#cbokepala_mekanik2').on('change', function() {
        var fieldValue = $(this).val();
        var fieldPercentage = $('#txtpersen_kepala2').val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('kepala_mekanik2', fieldValue, fieldPercentage);
        }
    });

    $('#txtpersen_kepala1').on('change blur', function() {
        var fieldValue = $('#cbokepala_mekanik1').val();
        var fieldPercentage = $(this).val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('kepala_mekanik1', fieldValue, fieldPercentage);
        }
    });

    $('#txtpersen_kepala2').on('change blur', function() {
        var fieldValue = $('#cbokepala_mekanik2').val();
        var fieldPercentage = $(this).val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('kepala_mekanik2', fieldValue, fieldPercentage);
        }
    ...
    // Auto-save Admin
    $('#cboadmin1').on('change', function() {
        var fieldValue = $(this).val();
        var fieldPercentage = $('#txtpersen_admin1').val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('admin1', fieldValue, fieldPercentage);
        }
    });
    
    $('#cboadmin2').on('change', function() {
        var fieldValue = $(this).val();
        var fieldPercentage = $('#txtpersen_admin2').val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('admin2', fieldValue, fieldPercentage);
        }
    });
    
    $('#txtpersen_admin1').on('change blur', function() {
        var fieldValue = $('#cboadmin1').val();
        var fieldPercentage = $(this).val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('admin1', fieldValue, fieldPercentage);
        }
    ...
    $('#txtpersen_admin2').on('change blur', function() {
        var fieldValue = $('#cboadmin2').val();
        var fieldPercentage = $(this).val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('admin2', fieldValue, fieldPercentage);
        }
    ...
    // Auto-save Mekanik
    $('#cbomekanik1').on('change', function() {
        var fieldValue = $(this).val();
        var fieldPercentage = $('#txtpersen_kerja1').val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('mekanik1', fieldValue, fieldPercentage);
        }
    });
    
    $('#cbomekanik2').on('change', function() {
        var fieldValue = $(this).val();
        var fieldPercentage = $('#txtpersen_kerja2').val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('mekanik2', fieldValue, fieldPercentage);
        }
    });
    
    $('#cbomekanik3').on('change', function() {
        var fieldValue = $(this).val();
        var fieldPercentage = $('#txtpersen_kerja3').val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('mekanik3', fieldValue, fieldPercentage);
        }
    });
    
    $('#cbomekanik4').on('change', function() {
        var fieldValue = $(this).val();
        var fieldPercentage = $('#txtpersen_kerja4').val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('mekanik4', fieldValue, fieldPercentage);
        }
    });
    
    $('#txtpersen_kerja1').on('change blur', function() {
        var fieldValue = $('#cbomekanik1').val();
        var fieldPercentage = $(this).val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('mekanik1', fieldValue, fieldPercentage);
        }
    ...
    $('#txtpersen_kerja2').on('change blur', function() {
        var fieldValue = $('#cbomekanik2').val();
        var fieldPercentage = $(this).val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('mekanik2', fieldValue, fieldPercentage);
        }
    ...
    $('#txtpersen_kerja3').on('change blur', function() {
        var fieldValue = $('#cbomekanik3').val();
        var fieldPercentage = $(this).val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('mekanik3', fieldValue, fieldPercentage);
        }
    ...
    $('#txtpersen_kerja4').on('change blur', function() {
        var fieldValue = $('#cbomekanik4').val();
        var fieldPercentage = $(this).val() || 0;
        if (fieldValue) {
            autoSaveMekanikData('mekanik4', fieldValue, fieldPercentage);
        }
...
<!-- Work Order Auto Input JavaScript is included in main file -->
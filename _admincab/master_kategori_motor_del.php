<?php
    ...
    // --------------------
		...
		$sql_preview = mysqli_query($koneksi, "SELECT * FROM tbkategori_motor WHERE id='$kd'");
		...
		if(isset($_POST['konfirmasi'])){
			if($action == 'delete') {
				// Cek apakah kategori motor masih digunakan di transaksi
				$cek_transaksi = mysqli_query($koneksi, "
					SELECT COUNT(*) as total FROM (
						SELECT 1 FROM tbtipe_motor WHERE id_kategori_motor = '$kd'
						UNION ALL
						SELECT 1 FROM tblbarang WHERE kategori_motor = '{$data_preview['kategori']}'
						UNION ALL
						SELECT 1 FROM tblservis WHERE kategori_motor = '{$data_preview['kategori']}'
					) as used_tables
				");
				$result_transaksi = mysqli_fetch_array($cek_transaksi);

				if($result_transaksi['total'] > 0) {
					$pesan_error = "KATEGORI MOTOR tidak bisa dihapus karena masih ada transaksi yang menggunakan kategori motor ini!";
				} else {
					// Hapus permanen
					$sql_delete = "DELETE FROM tbkategori_motor WHERE id='$kd'";
					$success_message = "Data berhasil dihapus";
				}
			} elseif($action == 'deactivate') {
				// Nonaktifkan
				$sql_delete = "UPDATE tbkategori_motor SET status='0' WHERE id='$kd'";
				$success_message = "Kategori Motor berhasil dinonaktifkan";
			} elseif($action == 'activate') {
				// Aktifkan
				$sql_delete = "UPDATE tbkategori_motor SET status='1' WHERE id='$kd'";
				$success_message = "Kategori Motor berhasil diaktifkan";
			}

			if(!isset($pesan_error)) {
				if(mysqli_query($koneksi, $sql_delete)) {
					echo "<script>alert('$success_message'); window.location='master_kategori_motor.php';</script>";
				} else {
...
<html lang="en">
	<head>
		...
		<meta name="description" content="<?php echo $title_action; ?> Master Kategori Motor" />
								...
								<div class="widget-box">
									<div class="widget-header widget-header-flat widget-header-small">
										<h5 class="widget-title">
											...
											<?php echo $title_action; ?> Master Kategori Motor
										</h5>
									...
									<div class="widget-body">
										<div class="widget-main">
											...
											<div class="alert alert-warning">
												...
												<p><?php echo ($action == 'delete') ? 'Anda akan menghapus PERMANEN data kategori motor berikut.' : 'Anda akan mengubah status kategori motor berikut.'; ?></p>
											</div>
														...
														<div class="widget-body">
															<div class="widget-main">
																<table class="table table-bordered table-striped">
																	<tbody>
																		<tr>
																			<td width="30%" class="text-right"><strong>Kategori Motor:</strong></td>
																			<td class="text-left">
																				<span class="label label-lg label-primary"><?php echo $data_preview['kategori']; ?></span>
																			</td>
																		</tr>
																		<tr>
											...
											<form method="post" class="form-horizontal" role="form">
												<div class="form-group">
													<div class="col-sm-12 text-center">
														<div class="alert alert-<?php echo ($action == 'delete') ? 'danger' : 'warning'; ?>">
															...
															<?php endif; ?>
															<p><strong>Apakah Anda yakin ingin <?php echo strtolower($title_action); ?> kategori motor "<span class="text-primary"><?php echo $data_preview['kategori']; ?></span>"?</strong></p>
														</div>
													</div>
												</div>
												...
												<div class="clearfix form-actions">
													<div class="col-md-12 text-center">
														...
														<a href="master_kategori_motor.php" class="btn btn-success btn-lg">

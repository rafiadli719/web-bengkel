<?php
    ...
    // --------------------
		...
		$tm_cari=mysqli_fetch_array($cari_kd);	
		$tgl_pesan=$tm_cari['tanggal_trx'];
		$drcabang=$tm_cari['kd_cabang'];
        $note=$tm_cari['note']; 
    ...
    // =========
        $cari_kd=mysqli_query($koneksi,"SELECT 
                                        sum(total) as tot_jual, 
                                        sum(quantity) as tot_qty_order 
                                        FROM tblorderjual_detail 
                                        WHERE 
                                        no_order='$nopesanan'");			
        $tm_cari=mysqli_fetch_array($cari_kd);
        $total_qty_order=$tm_cari['tot_qty_order'];                 
        $total_jual=$tm_cari['tot_jual'];                 

        $total_diskon=$total_jual*($diskon/100);                
        $total_pajak=$total_jual*($pajak/100);                
        ...
        if(isset($_POST['btnsimpan'])) {
            ...
            mysqli_query($koneksi,"INSERT INTO tblpenjualan_header 
                                    ...
                                    VALUES 
                                    ('$LastID','Penjualan','$cbocarabyr','$txttgljl',
            ...
            // Stok Barang
                ...
                while ($tampil = mysqli_fetch_array($sql)) {
                    ...
                    mysqli_query($koneksi,"INSERT INTO tbstok 
                                        ...
                                        ('3','$LastID','$no_item',
                                        '$txttgljl','0','$qty',
                                        'Penjualan Antar Cabang','$kd_cabang')"); 
                }
            // ----- End -----         

            // Pindahkan Data Penjualan ke Pesanan Pembelian

                // 1. Buat No Pesanan Pembelian ------
                $querycount="SELECT 
                ...
                // 2. Tarik Data dari penjualan Detail ke -> Pesanan Pembelian ------        
                $jml_pesan=0;
                $sql = mysqli_query($koneksi,"SELECT * FROM tblpenjualan_detail 
                                                WHERE no_transaksi='$LastID'");
                while ($tampil = mysqli_fetch_array($sql)) {
                    ...
                    $subtotal=$txtqty*$txthargabarang;
                    $jml_pesan=$jml_pesan+$subtotal;
                    
                    mysqli_query($koneksi,"INSERT INTO tblorder_detail 
                                            (no_order, no_item, 
                                            harga_pokok, quantity, 
                                            total, user, kd_cabang) 
                                            VALUES 
                                            ('$no_pesanan_pembelian', '$no_item',
                                            '$txthargabarang',
                                            '$txtqty','$subtotal',
                                            '$_nama','$kd_cabang')"); 
                ...
                // 3. Insert ke Pesanan Pembelian Header ------        
                mysqli_query($koneksi,"INSERT INTO tblorder_header 
                                        (no_order, status, tanggal, 
                                        no_supplier, total_qty, total_order, 
                                        user, kd_cabang, tipe_trx, no_penjualan) 
                                        VALUES 
                                        ('$no_pesanan_pembelian','0','$txttgljl',
                                        '$kd_cabang','$total_qty_order','$jml_pesan',
                                        '$_nama','$drcabang','Antar Cabang','$LastID')");
                        
            mysqli_query($koneksi,"UPDATE tblorderjual_header 
                                    SET status_pesanan='1' 
                                    WHERE no_order='$nopesanan'");                                                            

            echo"<script>window.alert('Proses Tarik Data Pesanan ke Penjualan telah berhasil!');
            window.location=('penjualan_cab_add.php');</script>";
        }        
?>
				...
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Proses Ke Penjualan</li>
						</ul><!-- /.breadcrumb -->
					</div>
					...
					<div class="page-content">
								...
								<div class="widget-box">
									<div class="widget-body">
										<div class="widget-main">	
                                            <br>
                                            <?php include "_template/_tarik_data_pesanan_header.php"; ?>
										</div>
									</div>
								</div>	
							</div>
								...
								<div class="widget-box">
									<div class="widget-body">
										<div class="widget-main">
                                            ...
                                            <?php include "_template/_tarik_data_pesanan_detail_mitra.php"; ?>
								...
								<div class="widget-box">
									<div class="widget-body">
										<div class="widget-main">
                                            ...
                                            <?php include "_template/_tarik_data_pesanan_lainnya_mitra.php"; ?>
								...
								<div class="widget-box">
									<div class="widget-body">
										<div class="widget-main">
                                            ...
                                            <?php include "_template/_tarik_data_pesanan_total_mitra.php"; ?>
						...
						<div class="row">
                                                ...
                                                <div class="col-xs-8 col-sm-6">
                                                    ...
                                                    onclick="return confirm('Proses Tarik Data Pesanan -> Ke Penjualan akan dibatalkan. Lanjutkan?')">                                                                    
...
<script type="text/javascript">
    $(document).ready(function() {
        $("#txtpotfaktur_persen").keyup(function() {
            ...
            var net = parseInt(subtotal)-parseInt(potfkt_nom)+parseInt(pajak_nom);
            var sisa = parseInt(net)-parseInt(dp);
                
            $("#txtpotfaktur_nom").val(potfkt_nom);
            $("#txtpajak_nom").val(pajak_nom);
            $("#txtnet").val(net);
            $("#txtkekurangan").val(sisa);
        });
    });
</script>
